// This file was auto-generated by Fern from our API Definition.

import * as AgentMail from "../../../src/api/index";
import { AgentMailClient } from "../../../src/Client";
import { mockServerPool } from "../../mock-server/MockServerPool";

describe("ApiKeysClient", () => {
    test("list (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });

        const rawResponseBody = {
            count: 1,
            next_page_token: "next_page_token",
            api_keys: [
                {
                    api_key_id: "api_key_id",
                    prefix: "prefix",
                    name: "name",
                    pod_id: "pod_id",
                    inbox_id: "inbox_id",
                    used_at: "2024-01-15T09:30:00Z",
                    permissions: {
                        read_inbox: true,
                        create_inbox: true,
                        update_inbox: true,
                        delete_inbox: true,
                        read_thread: true,
                        delete_thread: true,
                        read_message: true,
                        send_message: true,
                        update_message: true,
                        read_spam: true,
                        read_blocked: true,
                        read_trash: true,
                        read_draft: true,
                        create_draft: true,
                        update_draft: true,
                        delete_draft: true,
                        send_draft: true,
                        read_webhook: true,
                        create_webhook: true,
                        update_webhook: true,
                        delete_webhook: true,
                        read_domain: true,
                        create_domain: true,
                        update_domain: true,
                        delete_domain: true,
                        read_list_entry: true,
                        create_list_entry: true,
                        delete_list_entry: true,
                        read_metrics: true,
                        read_api_key: true,
                        create_api_key: true,
                        delete_api_key: true,
                        read_pod: true,
                        create_pod: true,
                        delete_pod: true,
                    },
                    created_at: "2024-01-15T09:30:00Z",
                },
                {
                    api_key_id: "api_key_id",
                    prefix: "prefix",
                    name: "name",
                    pod_id: "pod_id",
                    inbox_id: "inbox_id",
                    used_at: "2024-01-15T09:30:00Z",
                    permissions: {
                        read_inbox: true,
                        create_inbox: true,
                        update_inbox: true,
                        delete_inbox: true,
                        read_thread: true,
                        delete_thread: true,
                        read_message: true,
                        send_message: true,
                        update_message: true,
                        read_spam: true,
                        read_blocked: true,
                        read_trash: true,
                        read_draft: true,
                        create_draft: true,
                        update_draft: true,
                        delete_draft: true,
                        send_draft: true,
                        read_webhook: true,
                        create_webhook: true,
                        update_webhook: true,
                        delete_webhook: true,
                        read_domain: true,
                        create_domain: true,
                        update_domain: true,
                        delete_domain: true,
                        read_list_entry: true,
                        create_list_entry: true,
                        delete_list_entry: true,
                        read_metrics: true,
                        read_api_key: true,
                        create_api_key: true,
                        delete_api_key: true,
                        read_pod: true,
                        create_pod: true,
                        delete_pod: true,
                    },
                    created_at: "2024-01-15T09:30:00Z",
                },
            ],
        };
        server
            .mockEndpoint()
            .get("/v0/pods/pod_id/api-keys")
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.pods.apiKeys.list("pod_id");
        expect(response).toEqual({
            count: 1,
            nextPageToken: "next_page_token",
            apiKeys: [
                {
                    apiKeyId: "api_key_id",
                    prefix: "prefix",
                    name: "name",
                    podId: "pod_id",
                    inboxId: "inbox_id",
                    usedAt: new Date("2024-01-15T09:30:00.000Z"),
                    permissions: {
                        readInbox: true,
                        createInbox: true,
                        updateInbox: true,
                        deleteInbox: true,
                        readThread: true,
                        deleteThread: true,
                        readMessage: true,
                        sendMessage: true,
                        updateMessage: true,
                        readSpam: true,
                        readBlocked: true,
                        readTrash: true,
                        readDraft: true,
                        createDraft: true,
                        updateDraft: true,
                        deleteDraft: true,
                        sendDraft: true,
                        readWebhook: true,
                        createWebhook: true,
                        updateWebhook: true,
                        deleteWebhook: true,
                        readDomain: true,
                        createDomain: true,
                        updateDomain: true,
                        deleteDomain: true,
                        readListEntry: true,
                        createListEntry: true,
                        deleteListEntry: true,
                        readMetrics: true,
                        readApiKey: true,
                        createApiKey: true,
                        deleteApiKey: true,
                        readPod: true,
                        createPod: true,
                        deletePod: true,
                    },
                    createdAt: new Date("2024-01-15T09:30:00.000Z"),
                },
                {
                    apiKeyId: "api_key_id",
                    prefix: "prefix",
                    name: "name",
                    podId: "pod_id",
                    inboxId: "inbox_id",
                    usedAt: new Date("2024-01-15T09:30:00.000Z"),
                    permissions: {
                        readInbox: true,
                        createInbox: true,
                        updateInbox: true,
                        deleteInbox: true,
                        readThread: true,
                        deleteThread: true,
                        readMessage: true,
                        sendMessage: true,
                        updateMessage: true,
                        readSpam: true,
                        readBlocked: true,
                        readTrash: true,
                        readDraft: true,
                        createDraft: true,
                        updateDraft: true,
                        deleteDraft: true,
                        sendDraft: true,
                        readWebhook: true,
                        createWebhook: true,
                        updateWebhook: true,
                        deleteWebhook: true,
                        readDomain: true,
                        createDomain: true,
                        updateDomain: true,
                        deleteDomain: true,
                        readListEntry: true,
                        createListEntry: true,
                        deleteListEntry: true,
                        readMetrics: true,
                        readApiKey: true,
                        createApiKey: true,
                        deleteApiKey: true,
                        readPod: true,
                        createPod: true,
                        deletePod: true,
                    },
                    createdAt: new Date("2024-01-15T09:30:00.000Z"),
                },
            ],
        });
    });

    test("list (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });

        const rawResponseBody = { name: "name", message: "message" };
        server
            .mockEndpoint()
            .get("/v0/pods/pod_id/api-keys")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.pods.apiKeys.list("pod_id");
        }).rejects.toThrow(AgentMail.NotFoundError);
    });

    test("create (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });
        const rawRequestBody = { name: "name" };
        const rawResponseBody = {
            api_key_id: "api_key_id",
            api_key: "api_key",
            prefix: "prefix",
            name: "name",
            pod_id: "pod_id",
            inbox_id: "inbox_id",
            permissions: {
                read_inbox: true,
                create_inbox: true,
                update_inbox: true,
                delete_inbox: true,
                read_thread: true,
                delete_thread: true,
                read_message: true,
                send_message: true,
                update_message: true,
                read_spam: true,
                read_blocked: true,
                read_trash: true,
                read_draft: true,
                create_draft: true,
                update_draft: true,
                delete_draft: true,
                send_draft: true,
                read_webhook: true,
                create_webhook: true,
                update_webhook: true,
                delete_webhook: true,
                read_domain: true,
                create_domain: true,
                update_domain: true,
                delete_domain: true,
                read_list_entry: true,
                create_list_entry: true,
                delete_list_entry: true,
                read_metrics: true,
                read_api_key: true,
                create_api_key: true,
                delete_api_key: true,
                read_pod: true,
                create_pod: true,
                delete_pod: true,
            },
            created_at: "2024-01-15T09:30:00Z",
        };
        server
            .mockEndpoint()
            .post("/v0/pods/pod_id/api-keys")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(200)
            .jsonBody(rawResponseBody)
            .build();

        const response = await client.pods.apiKeys.create("pod_id", {
            name: "name",
        });
        expect(response).toEqual({
            apiKeyId: "api_key_id",
            apiKey: "api_key",
            prefix: "prefix",
            name: "name",
            podId: "pod_id",
            inboxId: "inbox_id",
            permissions: {
                readInbox: true,
                createInbox: true,
                updateInbox: true,
                deleteInbox: true,
                readThread: true,
                deleteThread: true,
                readMessage: true,
                sendMessage: true,
                updateMessage: true,
                readSpam: true,
                readBlocked: true,
                readTrash: true,
                readDraft: true,
                createDraft: true,
                updateDraft: true,
                deleteDraft: true,
                sendDraft: true,
                readWebhook: true,
                createWebhook: true,
                updateWebhook: true,
                deleteWebhook: true,
                readDomain: true,
                createDomain: true,
                updateDomain: true,
                deleteDomain: true,
                readListEntry: true,
                createListEntry: true,
                deleteListEntry: true,
                readMetrics: true,
                readApiKey: true,
                createApiKey: true,
                deleteApiKey: true,
                readPod: true,
                createPod: true,
                deletePod: true,
            },
            createdAt: new Date("2024-01-15T09:30:00.000Z"),
        });
    });

    test("create (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });
        const rawRequestBody = { name: "name" };
        const rawResponseBody = { name: "name", message: "message" };
        server
            .mockEndpoint()
            .post("/v0/pods/pod_id/api-keys")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.pods.apiKeys.create("pod_id", {
                name: "name",
            });
        }).rejects.toThrow(AgentMail.NotFoundError);
    });

    test("create (3)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });
        const rawRequestBody = { name: "name" };
        const rawResponseBody = { name: "name", errors: { key: "value" } };
        server
            .mockEndpoint()
            .post("/v0/pods/pod_id/api-keys")
            .jsonBody(rawRequestBody)
            .respondWith()
            .statusCode(400)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.pods.apiKeys.create("pod_id", {
                name: "name",
            });
        }).rejects.toThrow(AgentMail.ValidationError);
    });

    test("delete (1)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });

        server.mockEndpoint().delete("/v0/pods/pod_id/api-keys/api_key_id").respondWith().statusCode(200).build();

        const response = await client.pods.apiKeys.delete("pod_id", "api_key_id");
        expect(response).toEqual(undefined);
    });

    test("delete (2)", async () => {
        const server = mockServerPool.createServer();
        const client = new AgentMailClient({
            maxRetries: 0,
            apiKey: "test",
            environment: { http: server.baseUrl, websockets: server.baseUrl },
        });

        const rawResponseBody = { name: "name", message: "message" };
        server
            .mockEndpoint()
            .delete("/v0/pods/pod_id/api-keys/api_key_id")
            .respondWith()
            .statusCode(404)
            .jsonBody(rawResponseBody)
            .build();

        await expect(async () => {
            return await client.pods.apiKeys.delete("pod_id", "api_key_id");
        }).rejects.toThrow(AgentMail.NotFoundError);
    });
});
